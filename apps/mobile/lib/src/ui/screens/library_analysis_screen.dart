...
class _LibraryAnalysisScreenState extends State<LibraryAnalysisScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final list = ListView(
      ...
      padding: widget.embeddedInShell ? bookfolioShellTabScrollPadding(context) : bookfolioMobileScrollPadding(context),
      children: [
            Text(
            ...
            if (_loading)
              ...
              _AnalysisBlock(
                ...
                child: Text('내 서재의 책 수', style: tt.titleSmall?.copyWith(fontWeight: FontWeight.w700)),
              ),
              SizedBox(height: BookfolioDesignTokens.sectionGapSm),
              _AnalysisBlock(
                value: '${s.ownedWorkCount}권',
                caption: '시리즈는 제목 정규화로 묶은 작품 수(근사)',
                child: Text('내가 소장 중인 책 수', style: tt.titleSmall?.copyWith(fontWeight: FontWeight.w700)),
              ),
              SizedBox(height: BookfolioDesignTokens.sectionGapSm),
              Text('내 서재 현황', style: tt.titleSmall?.copyWith(fontWeight: FontWeight.w700)),
              ...
              _AnalysisBlock(
                ...
                child: Text('내 서재의 총 가격', style: tt.titleSmall?.copyWith(fontWeight: FontWeight.w700)),
              ),
              SizedBox(height: BookfolioDesignTokens.sectionGapSm),
              _AnalysisBlock(
                value: '${s.memoCount}개',
                child: Text('내가 기록한 메모', style: tt.titleSmall?.copyWith(fontWeight: FontWeight.w700)),
              ),
              SizedBox(height: BookfolioDesignTokens.sectionGapSm),
              _AnalysisBlock(
                value: '${s.oneLinerCount}개',
                child: Text('내가 기록한 한줄평', style: tt.titleSmall?.copyWith(fontWeight: FontWeight.w700)),
              ),
              SizedBox(height: BookfolioDesignTokens.sectionGapSm),
              Text('저자별 소장 TOP 3', style: tt.titleSmall?.copyWith(fontWeight: FontWeight.w700)),
                  ...
                  return Padding(
                    ...
                    child: ListTile(
                      ...
                      leading: CircleAvatar(
                        ...
                        child: Text('$rank', style: TextStyle(color: scheme.onPrimaryContainer, fontWeight: FontWeight.w800)),
                      ),
                      title: Text(a.name, maxLines: 2, overflow: TextOverflow.ellipsis),
                      trailing: Text('${a.count}권', style: tt.titleSmall?.copyWith(fontWeight: FontWeight.w700)),
                    ),
                  );
                }),
              ],
              if (!_loading) ...[
                ...
                if (_aggregateError != null)
                  Text(_aggregateError!, style: TextStyle(color: scheme.error, fontSize: 13))
                else if (_aggregatePayload != null) ...[
                  bookfolioAggregateSectionTitle(Theme.of(context), '소장 건수 순위'),
                  BookfolioMemberLeaderboardCard(data: _mapAggSection(_aggregatePayload!['ownedBooks']), unit: '권'),
                  bookfolioAggregateSectionTitle(Theme.of(context), '완독 순위'),
                  BookfolioMemberLeaderboardCard(data: _mapAggSection(_aggregatePayload!['completedBooks']), unit: '권'),
                  bookfolioAggregateSectionTitle(Theme.of(context), '소장 책 순위'),
                  BookfolioPopularBooksSection(raw: _aggregatePayload!['popularOwnedBooks']),
                  bookfolioAggregateSectionTitle(Theme.of(context), '포인트 순위'),
                  BookfolioMemberLeaderboardCard(data: _mapAggSection(_aggregatePayload!['points']), unit: 'P'),
                ] else
                  Text(
                    '집계 데이터를 불러오지 못했습니다.',
                    style: tt.bodySmall?.copyWith(color: scheme.onSurfaceVariant),
...
class _AnalysisBlock extends StatelessWidget {
  const _AnalysisBlock({required this.value, this.caption, required this.child});
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          value,
          style: tt.headlineSmall?.copyWith(fontWeight: FontWeight.w800, color: scheme.primary),
        ),
        if (caption != null) ...[
          const SizedBox(height: 6),
          Text(
            caption!,
            style: tt.bodySmall?.copyWith(color: scheme.onSurfaceVariant, height: 1.3),
          ),
        ],
      ],
    );
