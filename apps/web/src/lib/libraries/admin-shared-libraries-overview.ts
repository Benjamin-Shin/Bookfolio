...
function effectiveUserDisplayLabel(row: AppUserMini): string {
  ...
  const fromProfile = Array.isArray(prof)
    ? prof[0]?.display_name
    : prof?.display_name;
 ...
 */
export async function fetchAdminSharedLibrariesOverview(): Promise<
  AdminSharedLibraryOverviewRow[]
> {
  ...
  const [
    ...
    { data: lubRows, error: lubErr },
  ] = await Promise.all([
    supabase
      .from("library_members")
      .select("library_id,user_id,role")
      .in("library_id", libraryIds),
    supabase
      .from("library_user_books")
      .select("library_id,user_book_id")
      .in("library_id", libraryIds),
  ]);
  ...
  const membersByLibrary = new Map<
    string,
  ...
  const ubIds = [
    ...new Set((lubRows ?? []).map((r) => r.user_book_id as string)),
  ];
  ...
  return libraries.map((l) => {
    ...
    const creatorLabel = creator
      ? effectiveUserDisplayLabel(creator)
      : "(알 수 없음)";
