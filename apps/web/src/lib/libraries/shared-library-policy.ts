...
export class SharedLibraryCreateLimitReachedError extends Error {
  constructor(
    ...
    super(
      `모임서가는 최대 ${limit}개까지 만들 수 있습니다. (현재 ${createdCount}개 소유 중)`,
    );
...
export class SharedLibraryVipOwnedCapError extends Error {
  constructor(
    ...
    super(
      `VIP 플랜에서 모임서가는 최대 ${maxOwned}개까지 만들 수 있습니다. (현재 ${createdCount}개 소유 중)`,
    );
...
export class SharedLibraryPointsRequiredError extends Error {
  constructor(
    message = "포인트가 부족합니다. 추가 모임서가 또는 초대에 포인트가 필요합니다.",
  ) {
    super(message);
