...
export default async function AdminAuthorsPage({
  ...
  if (authorsResult.error) {
    return (
      <p className="text-sm text-destructive">
        저자 목록을 불러오지 못했습니다: {authorsResult.error.message}
      </p>
    );
  }
  ...
  if (linksResult.error) {
    return (
      <p className="text-sm text-destructive">
        연결 정보를 불러오지 못했습니다: {linksResult.error.message}
      </p>
    );
  }
  ...
  if (allIdsResult.error) {
    return (
      <p className="text-sm text-destructive">
        저자 ID 목록을 불러오지 못했습니다: {allIdsResult.error.message}
      </p>
    );
  }
  ...
  const linkedAuthorIds = new Set(
    (linkRows as BookAuthorLink[]).map((r) => r.author_id),
  );
  const orphanCount = (allIdsResult.data ?? []).filter(
    (r: { id: string }) => !linkedAuthorIds.has(r.id),
  ).length;
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-border/80 bg-muted/20 p-4">
        <p className="mb-2 text-sm text-muted-foreground">
          ...
          에 한 번도 연결되지 않은 저자는 DB 기준{" "}
          <span className="font-medium text-foreground">{orphanCount}</span>
          명입니다. 아래 버튼은 이들을 한 번에 삭제합니다(실행 시점에 다시
          집계). 아래 표는 검색 결과·최대 1000명까지입니다.
        </p>
