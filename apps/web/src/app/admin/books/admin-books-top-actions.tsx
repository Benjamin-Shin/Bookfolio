...
const NEW_BOOK_HREF = "/admin/books/new" as Route;
 ...
 */
export function AdminBooksTopActions({
  hasAladinConfig,
}: AdminBooksTopActionsProps) {
    ...
    <div className="flex min-w-0 flex-col items-end gap-2">
      <div className="flex flex-wrap justify-end gap-2">
        ...
        <form action={formAction} className="inline">
          <Button
            ...
            variant="secondary"
            size="sm"
            disabled={!hasAladinConfig || isPending}
          >
      ...
      {!hasAladinConfig ? (
        <p className="max-w-md text-right text-xs text-muted-foreground">
          ALADIN_API_BASE_URL을 설정하면, 오늘 베스트셀러·초이스 신간 중 아직
          books에 없는 ISBN만 일괄 등록할 수 있습니다.
        </p>
      ) : null}
      {state.error ? (
