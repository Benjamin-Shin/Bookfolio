 ...
 * - 2026-03-26: 신규
 */
export async function updatePointRuleFromForm(
  formData: FormData,
): Promise<void> {
  await requireAdmin();
  ...
  const monthly = parseOptionalNonNegInt(
    formData.get("monthlyCap")?.toString(),
  );
 ...
 * - 2026-03-26: 신규
 */
export async function createPointRuleFromForm(
  formData: FormData,
): Promise<void> {
  await requireAdmin();
  ...
  const eventCode =
    formData.get("eventCode")?.toString().trim().toLowerCase() ?? "";
