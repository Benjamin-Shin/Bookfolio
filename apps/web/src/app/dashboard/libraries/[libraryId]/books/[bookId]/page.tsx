...
export default async function SharedLibraryBookDetailPage({
  ...
  const book = await getLibraryAggregatedBook(
    libraryId,
    bookId,
    session.user.id,
    ctx,
              ...
              <tbody>
                {book.owners.map((o) => (
                  <tr key={o.userBookId} className="border-b border-border/60">
                    ...
                    <td className="py-2 pr-4">
                      {READING_STATUS_LABELS[o.readingStatus]}
                    </td>
                    <td className="py-2 text-muted-foreground">
                      {[o.location, o.memoPreview]
                        .filter(Boolean)
                        .join(" · ") || "—"}
                    </td>
                  </tr>
