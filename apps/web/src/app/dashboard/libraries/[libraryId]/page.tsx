...
type PageProps = {
  ...
  searchParams: Promise<{ genre?: string; owner?: string; tab?: string; page?: string }>;
};
...
function ownerHallOfFame(o: LibraryAggregatedBookRow["owners"][number]): boolean {
  return o.readingStatus === "completed" && o.rating != null && o.rating >= 4;
}

function bookMatchesTab(book: LibraryAggregatedBookRow, tab: LibraryBookTab): boolean {
  const { owners } = book;
  switch (tab) {
    case "owned":
      return true;
...
/**
 * 공동서재 상세(멤버·책) — 내 서재형 좌측 컬렉션·멤버 패널.
 *
 * @history
...
export default async function LibraryDetailPage({ params, searchParams }: PageProps) {
  ...
  const unreadPage = unreadTotal === 0 ? 1 : Math.min(pageRaw, unreadTotalPages);
  const completedPage = completedTotal === 0 ? 1 : Math.min(pageRaw, completedTotalPages);
  const hallPage = hallTotal === 0 ? 1 : Math.min(pageRaw, hallTotalPages);
  const ownedPage = ownedTotalAll === 0 ? 1 : Math.min(pageRaw, ownedTotalPages);
  ...
  if (tab === "completed" && completedTotal > 0 && pageRaw > completedTotalPages) {
    redirect(
      libraryDetailHref(libraryId, {
        tab: "completed",
        genre: genreFilter || undefined,
  ...
  const emptyTabMessage = (() => {
    if (books.length === 0) return "아직 이 서재에 올라온 책이 없습니다.";
    ...
    <div className="min-h-screen bg-[#fbf9f4] text-[#1b1c19] selection:bg-[#e9c176] selection:text-[#261900]">
      <div className="flex min-h-screen">
        <aside
          ...
          aria-label="공동서재 컬렉션·멤버"
        >
          <div className="mb-8">
            <h3 className="mb-1 font-sans text-[0.75rem] font-bold uppercase tracking-widest text-[#051b0e]">
              Collections
            </h3>
            <p className="font-sans text-[0.65rem] text-[#1a3021]/60">멤버 소장 기준</p>
          </div>
          <nav className="space-y-2">
            <Link href={sideHref("owned")} className={sideLinkClass(tab === "owned")}>
        ...
        <main className="flex-1 px-6 pb-24 pt-10 md:px-12 lg:ml-72 lg:px-16">
          <nav
            ...
            aria-label="공동서재 보기 탭"
          >
            {(
              [
                ["owned", `All (${ownedTotalAll})`, sideHref("owned"), tab === "owned"],
                ["reading", `Reading (${readingTotal})`, sideHref("reading"), tab === "reading"],
                ["unread", `To Read (${unreadTotal})`, sideHref("unread"), tab === "unread"],
                ["completed", `Done (${completedTotal})`, sideHref("completed"), tab === "completed"],
                ["hall", `HoF (${hallTotal})`, sideHref("hall"), tab === "hall"],
              ] as const
          ...
          <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <h1 className="font-sans text-2xl font-bold italic text-[#051b0e]">{lib.name}</h1>
              {lib.description ? (
                <p className="mt-1 max-w-2xl text-sm text-[#434843]">{lib.description}</p>
              ) : null}
            </div>
          ...
          <section className="space-y-6" aria-labelledby="lib-books-heading">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-baseline sm:justify-between">
              <h2 id="lib-books-heading" className="font-serif text-3xl text-[#051b0e]">
                {tabSectionLabel(tab)}
              </h2>
              {tab === "hall" ? (
                ...
                <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1 text-sm text-[#434843]">
                  ...
                  {tab === "owned" ? (
                    <span className="text-xs text-[#675d53]">
                      서재 전체 {books.length.toLocaleString("ko-KR")}권(합본 기준)
                    </span>
                  ) : null}
                </div>
                <LibraryGenreFilter
            ...
            {books.length === 0 ? (
              ...
              <>
                ...
                {tab === "reading" && listTotalForPager > READING_SHELF_LIMIT ? (
                  <p className="text-xs text-[#675d53]">
                    읽는 중인 책이 {listTotalForPager.toLocaleString("ko-KR")}권입니다. 앞선{" "}
                    {READING_SHELF_LIMIT.toLocaleString("ko-KR")}권만 표시합니다. ALL 탭에서 장르·소유자 필터로 좁혀 보세요.
                  </p>
                ) : null}
                {tab === "owned" || tab === "unread" || tab === "completed" || tab === "hall" ? (
