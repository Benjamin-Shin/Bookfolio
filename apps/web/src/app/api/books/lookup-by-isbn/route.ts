...
const ISBN_LOOKUP_PROVIDERS: readonly IsbnLookupExternalProvider[] = ["naver", "nl", "google"];
 ...
 *
 * @history
 * - 2026-03-24: 본문 `provider`(naver | nl | google) 선택 지원 — 관리자 폼에서 출처별 검색 분리
 */
  ...
  try {
    ...
    if (body.provider != null && body.provider !== "" && provider === undefined) {
      return NextResponse.json(
        { error: "provider는 naver, nl, google 중 하나이거나 생략해야 합니다." },
        { status: 400 }
      );
    }
