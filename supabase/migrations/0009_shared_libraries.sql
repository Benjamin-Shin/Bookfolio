-- 공동서재: 가족·모임 등. 물리적 한 권(library_books) + 멤버별 읽기 상태(library_book_member_states).
-- 개인 서재(user_books)와는 별도(A안). API(service role)에서 멤버십 검증.
...
comment on table public.libraries is '공동서재(가족·모임 등).';
comment on table public.library_members is '공동서재 멤버 및 역할(owner/member).';
comment on table public.library_books is '해당 서재 안의 물리적 한 권(공유 카탈로그 books 참조).';
comment on table public.library_book_member_states is '공동서재 책에 대한 멤버별 읽기 상태.';
