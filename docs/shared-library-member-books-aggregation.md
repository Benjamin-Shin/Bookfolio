# 공동서가: B-1 집계 + `library_books` 폐기 및 매핑 테이블
...
| 항목                             | 결정                                                                                                                                                                        |
| -------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 카탈로그 모델                    | **B-1만** 사용한다. 공동서가 목록은 **`user_books` 기반**이며, `books`의 `book_id`로 중복을 합친다.                                                                         |
| `library_books`                  | **제거한다.** 공동서가 전용 “한 권” 행·`added_by`·서가별 `location`/`memo` 등 **고유 기능은 없앤다.**                                                                       |
...
- 공동서가 화면에서는 **그 서가에 올려 둔** 멤버들의 책을 합쳐 보여 준다.
...
- 한 `user_book`은 여러 공동서가에 동시에 올릴 수 있음(행마다 `library_id`가 다름).
...
- 제품이 “멤버면 개인 서가 전체가 공동서가에 항상 반영”이면 `library_user_books` 없이  
...
| UI                          | 공동서가 상세가 `libraryBookId` 대신 **집계 행(예: `bookId`)** 또는 **소유자별 `userBookId`** 딥링크로 동작하도록 수정.                                                                                                                                  |

공동서가에만 있던 **위치·메모**가 필요하면:
...
- 공동서가와 개인 소장의 연결은 **`library_user_books (library_id, user_book_id)`** 로만 둔다.
