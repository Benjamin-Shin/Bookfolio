# 공동서재: B-1 집계 + `library_books` 폐기 및 매핑 테이블
...
| 항목 | 결정 |
|------|------|
| 카탈로그 모델 | **B-1만** 사용한다. 공동서재 목록은 **`user_books` 기반**이며, `books`의 `book_id`로 중복을 합친다. |
| `library_books` | **제거한다.** 공동서재 전용 “한 권” 행·`added_by`·서재별 `location`/`memo` 등 **고유 기능은 없앤다.** |
...
- 공동서재 화면에서는 **그 서재에 올려 둔** 멤버들의 책을 합쳐 보여 준다.
...
- 멤버십(`library_members`)만으로는 “가입했으니 내 서재 전체가 다 보인다”가 될 수 있다.  
  **책 단위로 공동서재에 올릴지** 정하려면 `library_id`와 **특정 `user_books` 행**을 연결하는 테이블이 필요하다.
- `library_books`는 `book_id`만 묶어 **어느 개인 소장 행과 대응하는지** 알 수 없었다. 매핑은 **`user_book_id` 단위**로 “이 권을 이 서재에 공유”를 표현한다.

### 3.2 스키마(제안)

```sql
-- 공동서재에 노출할 "개인 소장 한 줄"을 지정한다.
...
- 한 `user_book`은 여러 공동서재에 동시에 올릴 수 있음(행마다 `library_id`가 다름).
...
- 제품이 “멤버면 개인 서재 전체가 공동서재에 항상 반영”이면 `library_user_books` 없이  
...
| UI | 공동서재 상세가 `libraryBookId` 대신 **집계 행(예: `bookId`)** 또는 **소유자별 `userBookId`** 딥링크로 동작하도록 수정. |

공동서재에만 있던 **위치·메모**가 필요하면:

- **개인 필드로만** 쓴다: `user_books.location`, `user_books.memo` 유지.  
- “이 서재에서만 다른 메모”가 필요하면 **매핑 테이블에 `note text`** 같은 컬럼을 추가하는 확장만 허용한다 (`library_books` 부활 아님).
...
- 공동서재와 개인 소장의 연결은 **`library_user_books (library_id, user_book_id)`** 로만 둔다.  
- 목록은 이 매핑을 통해 잡은 `user_books`를 **`book_id`로 묶어** 소유자 이름을 붙여 표시한다.  
- “멤버 전체 서재 자동 공유”만 원하면 매핑 없이 구현할 수 있으나, **책 단위 공개**를 기본으로 하면 매핑 테이블이 필요하다.

---

*문서 기준 코드: `supabase/migrations/0009_shared_libraries.sql`, `apps/web/src/lib/libraries/repository.ts`, `packages/shared/src/contracts.ts`.*
