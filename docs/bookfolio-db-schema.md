...
`0001` … `0039` — 저장소 `supabase/migrations/` 디렉터리 (`0024`: 포인트 음수·VIP·출석·타임존·잔액 뷰, `0025`: 공동서재 추가·초과 초대 포인트 이벤트 시드, `0026`: 북폴리오 집계 RPC, `0029`–`0030`: 프로필·읽기 쪽수, `0031`: 발견 탭 커뮤니티 도서 RPC, `0032`: `app_profiles.onboarding_completed_at`, `0033`: 비밀번호 로그인 로컬 식별자 RPC, `0037`: `list_user_books_paged` 제목순 `p_sort`, `0038`: `list_user_books_paged` Hall of Fame `p_hall_of_fame`, `0039`: 추천 1·2차 로그/프로필/피처 벡터 테이블).
