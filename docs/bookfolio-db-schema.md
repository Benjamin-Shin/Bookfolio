...
| `app_profiles`                  | `app_users` 1:1 표시명·아바타·타임존·선택 인구통계 + 알라딘 관심 CID(`0042`) |
...
| `libraries`                     | 모임서가                                                                |
| `library_members`               | 모임서가 멤버·역할                                                      |
| `library_user_books`            | 모임서가에 노출할 `user_books` 매핑                                     |
| `library_events`                | 모임 일정·이벤트(`0043`)                                                |
| `library_event_rsvps`           | 일정별 멤버 RSVP(`0043`)                                                |
| `site_announcements`          | 전역 공지·헤더·목록용 (`0044`)                                          |
| `user_notifications`          | 사용자별 개인 알림·읽음 여부 (`0044`)                                   |
...
| `user_book_interactions`        | 사용자-도서 상호작용 로그(추천 1차) (`0039`)                            |
| `recommendation_impressions`    | 추천 노출/랭크/점수 로그(추천 1차) (`0039`)                             |
| `user_preference_profiles`      | 사용자 선호 프로필 집계(장르/저자/포맷 가중치, 추천 2차) (`0039`)       |
| `book_feature_vectors`          | 도서 피처 벡터/품질·인기도 점수(추천 2차) (`0039`)                      |
| `mobile_login_transfer_codes`   | 모바일 Bearer 세션→웹 NextAuth 세션 전이용 1회 코드(`0041`)             |
...
| `favorite_aladin_category_ids` | `integer[]`   | NOT NULL, default `'{}'` — 알라딘 국내도서 관심 카테고리 CID 최대 5개 (`0042`) |
...
| `name`        | `text`        | NOT NULL                                                                                       |
| `description` | `text`        |                                                                                                |
| `image_url`   | `text`        | 모임서가 대표 이미지 URL (`0040`)                                                              |
| `kind`        | `text`        | NOT NULL, CHECK `IN ('family','club')`                                                         |
| `created_by`  | `uuid`        | NOT NULL, FK → `app_users(id)` ON DELETE CASCADE (`0023` — 탈퇴 시 본인이 만든 서가 연쇄 삭제) |
| `created_at`  | `timestamptz` | NOT NULL, default `now()`                                                                      |
| `updated_at`  | `timestamptz` | NOT NULL, default `now()`                                                                      |
...
| `description` | `text`        |                                                                             |
| `location`    | `text`        |                                                                             |
| `event_kind`  | `text`        | NOT NULL, default `'meeting'`, CHECK `IN ('meeting','social','deadline')`   |
| `starts_at`   | `timestamptz` | NOT NULL                                                                    |
| `ends_at`     | `timestamptz` | NULL이면 시점 일정; NOT NULL일 때 `>= starts_at`                           |
| `created_by`  | `uuid`        | NOT NULL, FK → `app_users(id)` ON DELETE CASCADE                            |
| `cancelled_at`| `timestamptz` | 비NULL이면 취소(목록·캘린더에서 제외 권장)                                  |
| `created_at`  | `timestamptz` | NOT NULL, default `now()`                                                 |
| `updated_at`  | `timestamptz` | NOT NULL, default `now()` — `set_updated_at` 트리거                         |

인덱스: `(library_id, starts_at DESC)`, `starts_at`. RLS: 활성화, 직접 정책 없음(service_role 패턴).

---

## `public.library_event_rsvps` (`0043`)

| 컬럼               | 타입          | 제약 / 기본                                                                 |
| ------------------ | ------------- | --------------------------------------------------------------------------- |
| `library_event_id` | `uuid`        | PK(복합), FK → `library_events(id)` ON DELETE CASCADE                       |
| `user_id`          | `uuid`        | PK(복합), FK → `app_users(id)` ON DELETE CASCADE                            |
| `status`           | `text`        | NOT NULL, default `'pending'`, CHECK `IN ('going','maybe','declined','pending')` |
| `note`             | `text`        |                                                                             |
| `updated_at`       | `timestamptz` | NOT NULL, default `now()` — `set_updated_at` 트리거                         |

인덱스: `user_id`. RLS: 활성화, 직접 정책 없음(service_role 패턴).

---

## `public.site_announcements` (`0044`)

전역 공지. 게시 플래그·게시 시작·만료 시각으로 노출 여부를 제어한다. 웹은 `service_role`로 조회·관리 API 및 관리자 화면에서 접근한다.
...
| `published_at` | `timestamptz` | NULL — 게시 시작; 게시 중 목록은 NOT NULL·현재 시각 이후로 필터       |
| `expires_at`   | `timestamptz` | NULL — 만료 없음; NOT NULL이면 해당 시각 이후 비노출                   |
| `sort_order`   | `integer`     | NOT NULL, default `0` — 동일 조건 정렬 시 작은 값이 앞                |
| `created_by`   | `uuid`        | FK → `app_users(id)` ON DELETE SET NULL                               |
| `created_at`   | `timestamptz` | NOT NULL, default `now()`                                              |
| `updated_at`   | `timestamptz` | NOT NULL, default `now()` — `set_updated_at` 트리거                    |

CHECK: `expires_at`가 NULL이 아니면 `published_at`도 NULL이 아니어야 하며 `expires_at >= published_at`.

인덱스: `(is_published, sort_order, published_at DESC)`, `updated_at DESC`. RLS: 활성화, 정책 없음(service_role 패턴).

---

## `public.user_notifications` (`0044`)

사용자별 개인 알림. `read_at`이 NULL이면 미읽음. 행 삽입은 관리자 **`/admin/users`**(행별 알림)·**`POST /api/admin/user-notifications`**(`action: "create"`), 배치 등으로 수행한다. 전역 공지(`site_announcements`)와 무관하며 `/announcements`에는 노출되지 않는다. 웹 본인 조회·읽음 처리는 `/api/me/notifications`에서 `user_id`를 검증한 뒤 `service_role`로 접근한다.
...
| `title`     | `text`        | NOT NULL                                                                       |
| `body`      | `text`        | NOT NULL                                                                       |
| `kind`      | `text`        | NOT NULL, default `'info'`, CHECK `IN ('info','success','warning','system')`   |
| `read_at`   | `timestamptz` | NULL — 미읽음                                                                  |
| `metadata`  | `jsonb`       | NOT NULL, default `'{}'`                                                       |
| `created_at`| `timestamptz` | NOT NULL, default `now()`                                                      |

인덱스: `(user_id, created_at DESC)`, 부분 인덱스 `(user_id) WHERE read_at IS NULL`. RLS: 활성화, 정책 없음(service_role 패턴).
...
UNIQUE `(rule_version_id, event_code)`. 초기 이벤트(`0021`): `canon_edit_approved` 등(점수 0). `0022`·`0023`에서 `join_membership`(가입), `user_book_register`(서가 등록) 시드. `0025`에서 `shared_library_create_extra`·`shared_library_invite_extra`(차감) 시드 — 웹 관리 화면에서 점수·한도 수정 가능.
...
| `interaction_type`  | `text`          | NOT NULL, CHECK `impression \| click \| detail_view \| save \| dismiss \| start_read \| complete_read \| rate` |
| `interaction_value` | `numeric(10,4)` | NULL                                                                                                           |
| `surface`           | `text`          | NULL — 노출 위치(홈/발견/검색 등)                                                                              |
...
인덱스: `(user_id, occurred_at DESC)`, `(book_id, occurred_at DESC)`, `(interaction_type, occurred_at DESC)`, 부분 인덱스 `request_id`.  
RLS: 활성화, 정책 없음 — 앱은 `service_role` 경유 API에서 기록.

---

## `public.recommendation_impressions` (`0039`)
...
인덱스: `(user_id, served_at DESC)`, `(book_id, served_at DESC)`, 부분 인덱스 `request_id`, `(algorithm_version, experiment_bucket, served_at DESC)`.  
RLS: 활성화, 정책 없음 — 앱은 `service_role` 경유 API에서 기록.

---

## `public.user_preference_profiles` (`0039`)
...
| `format_weights`      | `jsonb`        | NOT NULL, default `'{}'`                   |
| `avg_rating`          | `numeric(4,2)` | NULL                                       |
| `completion_rate`     | `numeric(6,5)` | NOT NULL, default `0`                      |
| `recency_bias`        | `numeric(6,5)` | NOT NULL, default `0.5`                    |
| `profile_window_days` | `integer`      | NOT NULL, default `180`, CHECK `30..3650`  |
| `source_summary`      | `jsonb`        | NOT NULL, default `'{}'`                   |
| `updated_at`          | `timestamptz`  | NOT NULL, default `now()`                  |
| `created_at`          | `timestamptz`  | NOT NULL, default `now()`                  |

인덱스: `updated_at DESC`. RLS: 활성화, 정책 없음 — 배치/서버 API에서 upsert.

---

## `public.book_feature_vectors` (`0039`)
...
인덱스: `updated_at DESC`, `global_popularity_score DESC`. RLS: 활성화, 정책 없음 — 관리자/배치 API에서 upsert.

---

## `public.mobile_login_transfer_codes` (`0041`)

모바일 로그인 상태를 웹 로그인으로 넘길 때 쓰는 짧은 TTL(기본 60초) 1회용 코드 저장소.
...
인덱스: `user_id`, `expires_at`. RLS: 활성화, 정책 없음 — API는 `service_role`만 사용.
...
| `list_library_events_in_range(library_id, range_start, range_end_exclusive)` | 서비스 롤: 모임 일정 중 구간과 겹치는 `library_events` 행 집합 (`0043`)                                                                                                                                                         |
...
`0001` … `0043` — 저장소 `supabase/migrations/` 디렉터리 (`0024`: 포인트 음수·VIP·출석·타임존·잔액 뷰, `0025`: 모임서가 추가·초과 초대 포인트 이벤트 시드, `0026`: 북폴리오 집계 RPC, `0029`–`0030`: 프로필·읽기 쪽수, `0031`: 발견 탭 커뮤니티 도서 RPC, `0032`: `app_profiles.onboarding_completed_at`, `0033`: 비밀번호 로그인 로컬 식별자 RPC, `0037`: `list_user_books_paged` 제목순 `p_sort`, `0038`: `list_user_books_paged` Hall of Fame `p_hall_of_fame`, `0039`: 추천 1·2차 로그/프로필/피처 벡터 테이블, `0041`: 모바일→웹 로그인 세션 전이 1회 코드 테이블, `0042`: `app_profiles.favorite_aladin_category_ids`, `0043`: `library_events`·`library_event_rsvps`·`list_library_events_in_range`).
