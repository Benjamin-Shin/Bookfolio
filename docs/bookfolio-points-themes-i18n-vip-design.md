# 포인트·테마·i18n·VIP(구독)·모임서가·출석 — 통합 설계 요약
...
- **비구독:** 무료 한도 + 포인트 적립·차감으로 확장 기능(테마, 추가 모임서가, 추가 초대 등)을 연다.
...
| 모임서가             | `app_users.policies_json.sharedLibraryCreateLimit`(기본 1), `createLibrary`에서 검증 |
...
- 모임서가: `apps/web/src/lib/libraries/repository.ts`, `shared-library-policy.ts`
